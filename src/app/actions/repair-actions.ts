...
/**
 * Blog-repair orchestration for the "Analyse content" flow in the blog viewer.
 *
 * Given a blog's stored content + a fresh `BlogContentAnalysis`, asks Claude
 * (`repairBlogPost`) to rewrite it addressing every issue, and persists the
 * result as a side-by-side "After" version (no calendar entry).
 */
...
export async function repairBlogFromContent(
  ...
  try {
    ...
    const cleanMeta = normalizeMetaDescription(
      repaired.meta_description || blogRow.meta_description || '',
      sanitized.content,
    );
