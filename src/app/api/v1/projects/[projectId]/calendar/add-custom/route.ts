...
import { validateAddCustomPayload, invalidateCalendarCache } from "@/utils/calendar-validation";
...
export async function POST(req: Request, { params }: { params: Promise<{ projectId: string }> }) {
  ...
  try {
    ...
    const payloadResult = validateAddCustomPayload(bodyObj);
    ...
    const result = await addCustomKeywordToCalendar({
      ...
      writerNotes: payload.writerNotes,
    ...
    console.log(
      `[Telemetry] POST /api/v1/projects/${projectId}/calendar/add-custom: userId=${userId} duration=${duration}ms success=${success}`
    );
  }
}
