...
function AuditHistory({
  items, loading, projectId, onOpen, onGenerateFromHistory, onScheduleFromHistory,
}: {
  ...
  const [generatedBlogIds, setGeneratedBlogIds] = useState<Record<string, string>>({});
  const router = useRouter();

  useEffect(() => {
    const urlItems = items.filter(i => i.source !== "upload" && i.url && !i.url.startsWith("upload://"));
    if (!urlItems.length) return;
    void Promise.all(
      urlItems.map(async item => {
        try {
          const res = await fetch(
            `/api/v1/projects/${projectId}/content-audit/check-generated?url=${encodeURIComponent(item.url)}`
          );
          const data = (await res.json()) as { blogId?: string };
          if (data.blogId) {
            setGeneratedBlogIds(prev => ({ ...prev, [item.url]: data.blogId! }));
          }
        } catch { /* non-fatal */ }
      })
    );
              ...
              {isExpanded && (
                <div className="border-t border-border-subtle/50">
                  ...
                  <div className="px-4 py-3 border-t border-border-subtle/30 flex items-center gap-2 flex-wrap bg-surface-secondary/20">
                    {generatedBlogIds[item.url] ? (
                      <button
                        type="button"
                        onClick={() => router.push(`/projects/${projectId}/content-history/${generatedBlogIds[item.url]}`)}
                        className="h-8 px-3 rounded-[8px] bg-emerald-600 text-white text-[12px] font-semibold hover:bg-emerald-600/90 transition-all flex items-center gap-1.5"
                      >
