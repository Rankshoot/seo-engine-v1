...
export function buildBlogEnhancementPrompt(input: BlogEnhancementInput): string {
  ...
  return `You are an expert SEO content optimizer. Your task is to perform an enhancement pass on an existing blog post based on deep SERP analysis and SEO score issues.
...
SEO SCORE ISSUES (Failing checks):
${seoIssues.map((issue: any) => `- [${issue.key}] ${issue.label}: ${issue.hint}`).join("\n")}
...
export async function enhanceBlogFromDeepAnalysis(
  ...
  const responseText = await geminiGenerate(prompt, 3, false, "application/json");
  ...
  try {
    ...
    return {
      enhancedTitle: result.enhancedTitle || input.title,
      enhancedMetaDescription: result.enhancedMetaDescription || input.metaDescription,
      enhancedContentMarkdown: result.enhancedContentMarkdown || input.contentMarkdown,
      appliedFixes: Array.isArray(result.appliedFixes) ? result.appliedFixes : [],
      unresolvedIssues: Array.isArray(result.unresolvedIssues) ? result.unresolvedIssues : [],
      improvementSummary: result.improvementSummary || "",
    };
  } catch (err) {
    console.error("Failed to parse Gemini enhancement JSON:", err, "Raw response:", responseText);
